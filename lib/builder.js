...
ModuleSmith.prototype.build = function build(description, callback) {
  ...
  buildDomain.run(function () {
    async.waterfall([
      ...
      //
      // 4. Read and rewrite local files to handle
...
ModuleSmith.prototype.spawnNpm = function spawnNpm(description, callback) {
  var args = [description.command || 'install'],
      options,
      builder;

  if (description.command === 'build') { args.push('.') }
  if (description.options) { args = args.concat(description.options) }
  ...
  if (process.platform === 'win32') {
    delete options.uid;
    delete options.gid;
    delete options.env.USER;
    delete options.env.npm_config_user;
    builder = spawn('npm', args, options);
